//This code creates a favorites meal array in local storage if it does not already exist.
// It then has a function "fetchMealsFromApi" that takes in a url and a value, makes a request to that url with the value appended, and returns the JSON data. 
//The "showMealList" function takes the input value from a search bar and using the fetchMealsFromApi function, gets the meals that match that value. 
//It then displays the meals in cards on the main page with a button to show more details and a button to add or remove the meal from the favorites list. 
//The "showMealDetails" function takes in an id and makes a request to an API to get more information about the meal with that id and displays it on the main page.
